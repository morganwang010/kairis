...
const SalarySlipPage: React.FC<SalarySlipPageProps> = ({  projectId = 'all' }) => {
  ...
  const transformBackendData = (backendData: any[]): SalarySlipRecord[] => {
    return backendData.map(item => ({
      ...
      ew_wages: item.ew_wages + item.ew_add + item.ot_dirv|| 0,
      ot_wages: item.ot_wages + item.ot_add + item.ot_dirv || 0,
  ...
  const columns: TableColumnsType<SalarySlipRecord> = [
    ...
    {
      title: t('salarySlipPage.otWagesAllowance'),
      dataIndex: 'ot_wages',
      key: 'ot_wages',
      ellipsis: true,
      render: (amount) => <ScientificNumberDisplay value={amount} precision={0} />,
    ...
    {
      title: t('salarySlipPage.ewWagesAllowance'),
      dataIndex: 'ew_wages',
      key: 'ew_wages',
      ellipsis: true,
      render: (amount) => <ScientificNumberDisplay value={amount} precision={0} />,
